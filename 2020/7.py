...
bags_input="""dark orange bags contain 3 dark chartreuse bags.
...
# parse bags
bag_contents = {}
for line in bags_input.split("\n"):
    bag_color, contents = line.split(" bags contain ")
    bag_contents[bag_color] = {}
    for content in contents.strip(".").split(", "):
        if content != "no other bags":
            # 123 aaa bbb bags
            inner_bag_quantity = content.split()[0]
            inner_bag_color = " ".join(content.split()[1:-1])
            bag_contents[bag_color][inner_bag_color] = inner_bag_quantity

can_contain_shiny_gold = set()

left_to_check = []

for color, contents in bag_contents.items():
    if "shiny gold" in contents:
        left_to_check.append(color)

while len(left_to_check) > 0:
    containing_color = left_to_check.pop()
    can_contain_shiny_gold.add(containing_color)
    for color, contents in bag_contents.items():
        if containing_color in contents:
            left_to_check.append(color)

print(len(can_contain_shiny_gold))

shiny_gold_contains = 0
left_to_check = []
for color, count in bag_contents["shiny gold"].items():
    left_to_check += [color] * int(count)
    shiny_gold_contains += int(count)

while len(left_to_check) > 0:
    containing_color = left_to_check.pop()
    for color, count in bag_contents[containing_color].items():
        left_to_check += [color] * int(count)
        shiny_gold_contains += int(count)
