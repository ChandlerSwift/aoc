...
fn main() {
    ...
    let y_min = -92;
    let y_max = -53;
    let mut lands_in_target_count = 0;
    for x_velocity in 0..=x_max {
        for y_velocity in y_min..=-y_min {
            if lands_in_target(x_min, x_max, y_min, y_max, (x_velocity, y_velocity)) {
                lands_in_target_count += 1;
            }
        }
    }
    println!("{}", lands_in_target_count);
...
mod tests {
    ...
    fn find_how_many_velocities() {
        ...
        assert_eq!(lands_in_target_count, 112);
    }
}
