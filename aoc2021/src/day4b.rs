    ...
    for number_string in number_strings.split(',') {
        ...
        for i in 0..boards.len() {
            ...
            if check_win(boards[i]) {
                last_board_won = Some(boards[i]);
            } else {
                remaining_boards.push(boards[i]);
            }
        }
        boards = remaining_boards;
        if boards.len() == 0 { // Don't call the rest of the numbers if someone has already won
            break;
        }
    }

    let last_board_won = last_board_won.expect("No board has won?");
    println!("Won:\n{}\nScore: {}", last_board_won, last_board_won.score(number));
}

// TODO: this could be a static method of Board (e.g. Board::parse_board(...))
...
// TODO: this could be a method of Board
...
22 11 13  6  5
 2  0 12  3  7";
    ...
    fn find_last_win() {
        ...
        let mut last_board_won = None;
        let mut number = 0;
        let mut moves = RAW_MOVES.split(',');
        while boards.len() > 0 {
            let mut remaining_boards = Vec::new();
            number = moves.next().unwrap().parse().unwrap();
            for i in 0..boards.len() {
                boards[i].apply_move(number);
                println!("{}", boards[i]);
                if check_win(boards[i]) {
        ...
        assert_eq!(boards.len(), 0); // Every board has won
        let last_board_won = last_board_won.expect("No board has won?");
        assert_eq!(last_board_won.score(number), 1924);
    }
}
